...
<html lang="en">
   ...
   <?php 
    ...
    $res2 = mysqli_query($conn,"SELECT COUNT( docname ) FROM tbdoc where docstatus = '1'");
    $row2 = mysqli_fetch_row($res2);
    $sum2 = $row2[0];

    $res3 = mysqli_query($conn,"SELECT COUNT( docid ) FROM tbcancel");
   ...
   <body class="dashboard dashboard_1">
      <div class="full_container">
         <div class="inner_container">
            ...
            <?php 
               $dename = $_SESSION['dename'];
               $sql = "SELECT * FROM `tbreject` WHERE dename = '$dename' GROUP BY rejid ORDER BY rejby DESC LIMIT 5 ;";
               $result = mysqli_query($conn,$sql);
            ?>
                           <div class="card">
                              <div class="card-body">
                                 ...
                                 <div class="msg_list_main">
                                    ...
                                    <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                                       <li>
                                          <span>
                                          <span class="name_user"><?php echo $row['rejby'] ?></span>
                                          <span class="msg_user">ไฟล์ :  <?php echo $row['docid'] ?></span><br>
                                          <span class="msg_user">หมายเหตุ : <?php echo $row['remark'] ?></span>
                                          </span>
                                          <a href="viewrej.php?doc_id=<?php echo $row['docid'] ?>" class=" btn fa fa-trash"></a>
                                       </li>
                                       <?php endwhile; ?>
                                       <?php
                                else :
